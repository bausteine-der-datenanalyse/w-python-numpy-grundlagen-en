...
By default, the `np.loadtxt()` function expects whitespace-separated values. If the data is separated by a different character, the `delimiter = ""` option can be used to specify a different delimiter. For example, if the separator is a semicolon, the function call would look like:
